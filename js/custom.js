...
document.addEventListener('DOMContentLoaded', function() {
  const swiperEl = document.querySelector('.fullpage-swiper');
  if (!swiperEl) return;
  ...
  const swiper = new Swiper(swiperEl, {
    ...
    mousewheel: {
      enabled: true,
      releaseOnEdges: false, // Force single scroll logic
    },
    speed: 1000,
    keyboard: {
      enabled: true,
    ...
    on: {
      init: function() {
        ...
        gsap.set('.swiper-slide-active .banner-title-area, .swiper-slide-active h1, .swiper-slide-active h2, .swiper-slide-active .single-process', {
          autoAlpha: 0,
          y: 50
        });
        animateSlideIn(this);
      ...
      slideChangeTransitionStart: function() {
        // Exit animations for outgoing slide
        const prevIndex = this.previousIndex;
        const prevSlide = this.slides[prevIndex];
        gsap.to(prevSlide.querySelectorAll('h1, h2, .banner-title-area, .single-process'), {
          autoAlpha: 0,
          y: -50,
          duration: 0.5,
          ease: 'power2.in'
      ...
      slideChangeTransitionEnd: function() {
        // Entrance animations for active slide
        animateSlideIn(this);
      }
    }
  });
  ...
  function animateSlideIn(swiper) {
    ...
    gsap.fromTo(activeSlide.querySelectorAll('h1, h2, .banner-title-area, .single-process'), 
      { autoAlpha: 0, y: 50 },
...
// });
  ...
  if (document.querySelector(".pf-horizontal-slider")) {
    window.pfHorizontalSwiper = new Swiper(".pf-horizontal-slider", {
      loop: false,
      modules: [SwiperGL],
      speed: 1200,
      effect: "gl",
 ...
 gsap.registerPlugin(Observer);
        ...
        function updateCircleProgress() {
            if (!progressPath || maxIndex <= 0) return;
            const progress = pathLength - (current / maxIndex) * pathLength;
            progressPath.style.strokeDashoffset = progress;
        }
        ...
        function gotoSection(index) {
            ...
            // If we are moving to the swiper section (index 2)
            if (index === 2 && window.pfHorizontalSwiper) {
                if (current < index) {
                    // Coming from above, go to first slide
                    window.pfHorizontalSwiper.slideTo(0, 0);
                } else if (current > index) {
                    // Coming from below, go to last slide
                    window.pfHorizontalSwiper.slideTo(window.pfHorizontalSwiper.slides.length - 1, 0);
                }
            }

     if (index > current) {
        // Moving down → hide header
        header.classList.add("hide-header");
    } else if (index < current) {
        // Moving up → show header
     ...
     if (index === 0) {
        ...
        header.classList.add("scrolled-bg");
            ...
            gsap.to(content, {
                ...
                onComplete: () => {
                    ...
                    animateRevealText(sections[current]);
                }
            });
        }
        ...
        Observer.create({
            ...
            onDown: (self) => {
                ...
                // Wheel Down = Next (forward=true)
                // Touch Down (Swipe Down) = Prev (forward=false)
                const isWheel = self.event.type === "wheel";
                const forward = isWheel ? true : false;

                if (current === 2 && window.pfHorizontalSwiper) {
                    if (window.pfHorizontalSwiper.animating) return;
                    if (forward) {
                        if (!window.pfHorizontalSwiper.isEnd) {
                            window.pfHorizontalSwiper.slideNext();
                            return;
                        }
                    } else {
                        if (!window.pfHorizontalSwiper.isBeginning) {
                            window.pfHorizontalSwiper.slidePrev();
                            return;
                        }
                    }
                }
                gotoSection(current + (forward ? 1 : -1));
            },
            onUp: (self) => {
                if (isAnimating) return;
                // Wheel Up = Prev (forward=false)
                // Touch Up (Swipe Up) = Next (forward=true)
                const isWheel = self.event.type === "wheel";
                const forward = isWheel ? false : true;
        ...
        function splitTextNodes(node) {
            if (node.nodeType === 3) { // Text node
                ...
                // Skip empty text nodes (e.g. whitespace between tags)
        ...
        function animateRevealText(section) {
            if (!section) return;
            const textElements = section.querySelectorAll(".text-revel-anim");
            const text = section.querySelectorAll(".animated-text");

            const partnerListItems = section.querySelectorAll(".partner-list li");
            if (partnerListItems.length > 0) {
                 gsap.fromTo(partnerListItems,
                    { opacity: 0, y: 30 },
            ...
            textElements.forEach(text => {
                if (text.classList.contains("animated")) return;
            ...
            text.forEach(text => {
                if (text.dataset.animInitialized) return;
                text.dataset.animInitialized = true;
                gsap.to(text, {
                    opacity: 1,
                    y: 0,
                    duration: 1,
        ...
        // Trigger animation on load for the initial section
        window.addEventListener("load", () => {
